                ...
                <td className="px-5 py-4">
                  <div className="flex flex-wrap gap-1">
                    {role.permissions.map((p) => (
                      <span key={p} className="text-[10px] px-2 py-0.5 rounded bg-white/5 text-[var(--admin-text-muted)]">
                        {p}
                      </span>
                    ))}
                  </div>
            ...
            >
              <div className="bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-2xl w-full max-w-xl max-h-[85vh] flex flex-col shadow-2xl">
                <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--admin-border)]">
                  ...
                  <button onClick={() => setShowModal(false)} className="p-1.5 rounded-md text-[var(--admin-text-muted)] hover:text-[var(--admin-text-primary)] hover:bg-white/5 transition-colors">
                    <X size={16} />
                  </button>
                </div>
                    ...
                    <div key={group.group}>
                      ...
                      <div className="bg-[var(--admin-bg-secondary)] border border-[var(--admin-border)] rounded-lg divide-y divide-[var(--admin-border)]">
                        {group.perms.map((perm) => (
                          <label key={perm.key} className="flex items-center justify-between px-4 py-2.5 cursor-pointer hover:bg-white/5 transition-colors">
                ...
                <div className="px-6 py-4 border-t border-[var(--admin-border)] flex gap-3">
                  <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 rounded-lg border border-[var(--admin-border)] text-sm text-[var(--admin-text-muted)] hover:bg-white/5 transition-colors">
                    İptal
                  </button>
                  <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 rounded-lg bg-[var(--admin-accent)] hover:bg-[var(--admin-accent-hover)] text-black font-semibold text-sm transition-colors flex items-center justify-center gap-2">
                    <Check size={14} />
                    Kaydet
                  </button>
                </div>
